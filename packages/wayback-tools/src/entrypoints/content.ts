...
export default defineContentScript({
  ...
  async main() {
    ...
    const metaContainer = container?.querySelector(
      '.work.meta.group'
    ) as HTMLDListElement;
    const metaList = [];
    if (metaContainer) {
    ...
    const workContainer = container?.querySelector(
      '#workskin'
    ) as HTMLDivElement;
    ...
    if (workContainer) {
      metaData.title =
        workContainer.querySelector('.title.heading')?.textContent ?? '未知';
      metaData.author =
        workContainer.querySelector('.author')?.textContent ?? '未知';
      metaData.language =
        metaContainer.querySelector('dd.language')?.textContent ?? '未知';
      metaData.description =
        workContainer.querySelector('.summary ')?.textContent ?? '';
    }
    const book = new EpubBook(metaData);
    const chaptersContainer = container?.querySelector('#chapters') as
      | HTMLDivElement
      | undefined;
